...
async def run_benchmark(name, scheduler):
    ...
    while scheduler.queue:
        if batch_i % MEM_SAMPLE_EVERY == 0:
            peak_mem = max(peak_mem, get_gpu_mem_mb())
        batch_i += 1

        batch = scheduler.next_batch()
        batch_res = await asyncio.gather(
            *[run_request(r) for r in batch]
        )
    ...
    tpot = np.array([
        ((r["latency"] - r["ttft"]) * 1000) / r["tokens"]
        for r in results
        if r["ttft"] and r["tokens"] > 0
    ])

    summary = {
        "scheduler": name,
        "throughput_req_s": len(results) / (end - start),
        "avg_ttft_ms": ttft.mean(),
        "avg_tpot_ms": tpot.mean(),
...
async def main():
    ...
    with open("benchmark_results_fixed.json", "w") as f:
